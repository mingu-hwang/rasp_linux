	...
	(struct snd_pcm_substream *substream)
{
	...
	asoc_rtd_to_codec(rtd, 0)->driver->capture.channels_max = 2;
	...
	(struct snd_soc_pcm_runtime *rtd)
{
	...
	unsigned int fixed_bclk_ratio = 64; // Fixed bclk ratio (64fs)

	ret = snd_soc_dai_set_bclk_ratio(cpu_dai, fixed_bclk_ratio);
	pr_info("[MINGU] %s: fixed_bclk_ratio: %d", __func__, fixed_bclk_ratio);
	return ret;
}
...
static const struct snd_soc_dapm_route audioinjector_audio_map[] = {
	...
	/* mic is connected to Mic Jack, with max98390 Mic Bias */
